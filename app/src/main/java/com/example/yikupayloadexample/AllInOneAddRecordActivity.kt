...
class AllInOneAddRecordActivity: AppCompatActivity() {
...
//            mUploadPlan.visibility = View.VISIBLE
        ...
        thread {
            try {
                ...
                if (allInOneService.uploadFile(uploadFile, callback)) {
...
//                    uploadFile()
                ...
                try {
                    thread {
                        ...
                        allInOneService.uploadFile(sendDataArr, object : UploadFileCallback {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        mUploadBtn.setOnClickListener {
            // 如果uploadFile未被初始化，说明没有选择文件，先跳转到选择文件页面
            if(!::uploadFile.isInitialized) {
                EasyFloat.Companion.hide("yk_shout_weight_op")
                val intent = Intent(Intent.ACTION_GET_CONTENT)
                //任意类型文件
                intent.type = "audio/*"
                intent.addCategory(Intent.CATEGORY_OPENABLE)
            ...
            if (allInOneService.platform == VehiclePlatform.M300) {
                socketUploadFile()
            } else {
                httpUploadFile()
            }
