...
import com.yiku.yikupayloadSDK.util.Thrower200Host
...
class Thrower200Weight(context: Context, attr: AttributeSet?, defStyleAttr: Int) :
    LinearLayout(context, attr, defStyleAttr) {
    private val TAG = "Thrower200Weight"
...
//    private lateinit var mTemperature: TextView
    ...
    init {
        ...
        val host = preferences?.getString("Thrower200Host", "")
        if(host != null && "" != host) {
            throwerService.setIp(host)
        }
        else {
            throwerService.setIp(Thrower200Host)
        }
        initView(context)
        throwerService.registMsgCallback(object : MsgCallback {
            override fun getId(): String {
                return "Thrower200WeightCallback"
            }
    ...
    fun toOpen(index: Int) {
        if (!mThrowerSafetySwitch.isChecked) {
            ...
            val isCanDetonate = canDetonate_1 && canDetonate_2 && canDetonate_3 && canDetonate_4
    ...
    private fun initView(context: Context) {
        LayoutInflater.from(context).inflate(R.layout.thrower200_weight, this, true)
