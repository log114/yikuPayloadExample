...
public class RoundMenuView extends View {
    ...
    private void startLongPressOperation(int status) {
        new Thread(() -> {
            while (isLongPress) {
                roundMenus.get(status).onClickListener.onClick(this);
                try {
                    Thread.sleep(50);
    ...
    private void stopLongPressOperation() {
        isLongPress = false;
        // 停止长按操作的逻辑，比如取消线程或定时器
    }
        ...
        switch (event.getAction()) {
            case MotionEvent.ACTION_DOWN:
                ...
                if (distanceLine <= roundRadius) {
                    ...
                    isLongPress = true;
                    startLongPressOperation(onClickState);
                } else {
                    //点击了外面
                    onClickState = -2;
                }
